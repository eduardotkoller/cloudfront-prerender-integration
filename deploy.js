if (!process.env["CLOUDFRONT_DISTRIBUTION_ID"]) {
  ...
  const lambdaMappings = [
    {
      FunctionName: "Lambda-Edge-Prerenderio-dev-viewerRequest",
      EventType: "viewer-request"
    },
    {
      FunctionName: "Lambda-Edge-Prerenderio-dev-originRequest",
  ...
  const updateCloudFront = (cloudFrontId, lambdaMappings) =>
    cloudfront.getDistributionConfig({ Id: cloudFrontId }).promise().then(res => {
      console.log(
        "cloudfront config lambdaAssociations before",
      ...
      console.log(
        "cloudfront config lambdaAssociations after",
        res.DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations
          .Items
      );
